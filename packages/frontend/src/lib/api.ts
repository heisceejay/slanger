...
export async function suggestInventory(lang: Language): Promise<{ language: Language; rationale: string }> {
  ...
  const mergedPhonology = {
    ...
    writingSystem: hasWS
      ? {
        ...
        // we keep the old glyphs, they will be regenerated by Phonology.tsx
      }
  ...
  const finalLang: Language = {
    ...lang,
    phonology: mergedPhonology,
  };

  return {
    language: persistWithHistory(lang, finalLang, "Before: Suggest phoneme inventory"),
