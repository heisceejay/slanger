 ...
 */
import type {
  PhonologyConfig, PhonemeInventory, Phonotactics, AllophonyRule, WritingSystemConfig
...
export function validateWordForm(form: string, phonotactics: Phonotactics, inventory: PhonemeInventory): WordFormValidationResult {
  ...
  for (const { onset, nucleus, coda } of rawSylls) {
    ...
    if (!matched) {
      issues.push({
        ruleId: "PHON_012", severity: "error",
...
/**
 * Generates a writing system configuration based on the inventory and stylistic parameters.
 */
export function generateWritingSystem(
  inventory: PhonemeInventory,
  type: WritingSystemConfig["type"],
  aesthetics: WritingSystemConfig["aesthetics"]
): WritingSystemConfig {
  const mappings: Record<string, string[]> = {};
  const allPhonemes = [...inventory.consonants, ...inventory.vowels, ...inventory.tones];

  // Basic ASCII-based placeholder generation for now
  // In a real system, this would use a glyph generator or SVGs
  const graphemes = "abcdefghijklmnopqrstuvwxyzþðßŋχħʕʔ".split("");

  allPhonemes.forEach((ph, i) => {
    if (type === "abjad" && inventory.vowels.includes(ph)) {
      mappings[ph] = []; // Omit vowels in abjads
    } else {
      mappings[ph] = [graphemes[i % graphemes.length] ?? ph];
    }
  });
  ...
  return {
    ...
    mappings,
    aesthetics,
    glyphs: {} // TBD: abstract glyph descriptions
  };
}
...
export function validatePhonologyConfig(config: PhonologyConfig): PhonologyValidationIssue[] {
  ...
  if (config.writingSystem) {
    issues.push(...validateWritingSystem(config.writingSystem, config.inventory));
  }

  return issues;
}

export function validateWritingSystem(config: WritingSystemConfig, inventory: PhonemeInventory): PhonologyValidationIssue[] {
  const issues: PhonologyValidationIssue[] = [];
  const allPhonemes = new Set([...inventory.consonants, ...inventory.vowels, ...inventory.tones]);

  // Mapping checks
  const mappedPhonemes = Object.keys(config.mappings);
  for (const ph of mappedPhonemes) {
    if (config.type !== "syllabary" && !allPhonemes.has(ph)) {
      issues.push({ ruleId: "WS_001", severity: "warning", message: `Writing system maps unknown phoneme /${ph}/.`, entityRef: ph });
    }
  }

  if (config.type === "alphabet" || config.type === "abjad" || config.type === "abugida") {
    const missing = [...allPhonemes].filter(ph => !config.mappings[ph]);
    if (config.type === "abjad") {
      // Abjads typically omit vowels, so only warn if consonants are missing
      const missingConsonants = missing.filter(ph => inventory.consonants.includes(ph));
      if (missingConsonants.length > 0) {
        issues.push({ ruleId: "WS_002", severity: "error", message: `Abjad missing consonant mappings: ${missingConsonants.join(", ")}` });
      }
    } else {
      if (missing.length > 0) {
        issues.push({ ruleId: "WS_003", severity: "error", message: `Writing system missing mappings for phonemes: ${missing.join(", ")}` });
      }
    }
  }

  // Aesthetic sanity
  if (config.aesthetics.complexity < 0 || config.aesthetics.complexity > 1) {
    issues.push({ ruleId: "WS_010", severity: "warning", message: "Aesthetic complexity should be between 0 and 1." });
  }
...
export function tokenizeIpa(form: string, inventory: string[]): string[] | null {
