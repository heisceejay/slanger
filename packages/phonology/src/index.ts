...
export function generateWritingSystem(
  ...
  // We use a set of base grapheme identifiers (names for glyphs)
  ...
  allPhonemes.forEach((ph, i) => {
    ...
    if (type === "abjad" && isVowel) {
      mappings[ph] = [];
    } else {
      const g = graphemes[i % graphemes.length] ?? ph;
      mappings[ph] = [g];
      if (!glyphs[g]) {
        glyphs[g] = generateProceduralPath(g, aesthetics);
      }
    }
  });
...
function generateProceduralPath(seed: string, aes: WritingSystemConfig["aesthetics"]): string {
  ...
  // Starting point within a 32x32 viewbox (centered-ish)
  let x = 8 + rnd() * 16;
  let y = 8 + rnd() * 16;
